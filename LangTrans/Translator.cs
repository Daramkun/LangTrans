...
namespace LangTrans
{
	...
	public static class Translator
	{
		...
		public static bool Translate ( Language sourceLanguage, Language destinationLanguage, string text )
		{
			if ( sourceLanguage == Language.Unknown || destinationLanguage == Language.Unknown ) return false;
			if ( text.Trim ().Length == 0 ) return false;

			System.Net.HttpWebRequest hwr = System.Net.HttpWebRequest.Create ( String.Format (
				"http://translate.google.com/m?hl={1}&sl={0}&tl={1}&ie=UTF-8&prev=_m&q={2}",
				m_langDics [ sourceLanguage ], m_langDics [ destinationLanguage ], text ) ) as System.Net.HttpWebRequest;
			hwr.UserAgent = "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1;.NET CLR1.1.4322; InfoPath.1; .NET CLR 2.0.50727)";
			hwr.BeginGetResponse ( ( IAsyncResult ar ) =>
			{
				HttpWebResponse hwrr = hwr.EndGetResponse ( ar ) as System.Net.HttpWebResponse;
				TextReader tr = new StreamReader ( hwrr.GetResponseStream (), Encoding.UTF8 );
				string responsed = tr.ReadToEnd ();
